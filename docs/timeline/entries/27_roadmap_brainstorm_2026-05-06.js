export const entry27 = {
  ...
  summary:
    ...
    "(merchant-POV, the isekai-life-sim anime-raised-AI-life-sim-style, life-sim-like, combining-magic, a 4X strategy), reframed " +
    "combining-logic as universal compositional pattern (not magic-specific), recognized that the isekai-life-sim anime " +
    "the raised-AI life-sim is essentially a life-sim setup. Decided next move: merchant-POV game as pipeline " +
    "freshness test + combining-logic skill shakedown, then raised-AI-life-sim-style as second target after " +
  ...
  highlights: [
    ...
    "<strong>Game idea — merchant-POV (merchant-game-like)</strong>: BEST next pipeline test. NPC-side simulation, async news events, quest-issuing-not-receiving. Yume's relation/signal primitives handle this naturally. Honest scope: 1 shop, ~5 traveler types, ~10 item types, news as world.signal. Simultaneously tests pipeline freshness + the new combining-logic skill (merchant crafts items by combining materials).",
    "<strong>Game idea — the raised-AI life-sim anime arc-style life-sim</strong>: the the raised-AI life-sim arc (LN vols 9-18, 2018-2020 anime) is the Underworld project — researchers raise artificial souls (Fluctlights) from infancy through full life arcs in a medieval-village simulation. Two POV options: (a) play the Fluctlight (life sim from inside) or (b) play the researcher (god-game observer). Two unique design levers: time-compression (Fluctlights live decades, observers see days) and the dual-POV framing. Yume substrate suits this — harvestcore already has NPCs with schedules + needs + relationships.",
    "<strong>Game idea — life-sim-like</strong>: harvestcore + needs systems + select-and-direct UI. Substrate mostly there.",
    "<strong>Game idea — a 4X strategy</strong>: too big without Tier 3 actors (faction AI). Premature.",
    ...
    "<strong>the trapped-in-MMO arc (the trapped-in-MMO anime arcs)</strong>: noted but unbuildable without picking the GAME mechanic — combat? floor-clearing? guild management? 'Build the isekai-life-sim anime' is too underspecified. the raised-AI life-sim is the buildable arc because life-sim IS the mechanic.",
    ...
    "<strong>Decision — second move</strong>: the isekai-life-sim anime-raised-AI-life-sim-style life-sim AFTER answering the time-compression engine question (do we add a year_counter world_state binding? variable tick_seconds? compress via tick interval?). Needs design conversation, not a clean /yume-design drop-in.",
