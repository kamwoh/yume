	...
	for r in rules_array:
		...
		if raw_effect != null:
			...
			if expanded.size() > MAX_EXPANDED_EFFECTS:
				push_warning(
					(
						"Macro expansion: rule '%s' expanded to %d effects (limit %d) — keeping first %d"
						% [
							str(rule_dict.get("id", "")),
							expanded.size(),
							MAX_EXPANDED_EFFECTS,
							MAX_EXPANDED_EFFECTS
						]
					)
				)
...
func _expand_one(eff: Dictionary, depth: int, env: Dictionary, rule_id: String) -> Array:
	if depth > MAX_RECURSION_DEPTH:
		push_warning(
			(
				"Macro expansion: depth limit (%d) exceeded in rule '%s'"
	...
	for sub_template in template:
		if not (sub_template is Dictionary):
			continue
