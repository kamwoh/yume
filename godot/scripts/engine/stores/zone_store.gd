			...
			if str(child.get("parent", "")) != "":
				errors.append(
					EngineError.raise(
						...
						"zone.multi_parent",
						(
							"zone '%s' contained by both '%s' and '%s'"
							% [child_id, child["parent"], zid]
						),
						{
							"file": "world/zones.json",
							"child": child_id,
...
func find(spec: Dictionary) -> Array:
	...
	if spec.has("id"):
		...
		if not zones.has(zid):
			return []
		if not _matches_filters(zid, spec):
			return []
		return [zid]
...
func _matches_filters(zone_id: String, spec: Dictionary) -> bool:
	...
	if spec.has("type"):
		if str(z.get("type", "")) != str(spec["type"]):
			return false
	if spec.has("state"):
