extends Object
class_name EffectAdrExtensions

## ADR-specific effect handlers — late-phase primitives added by
## Phase-2/3/4 ADRs that build on the core vocabulary:
##
##   - build_place + _apply_chain — ADR 0037 grid-based placement
##   - switch_class — ADR 0030 class/occupation primitive
...
##     transition_player_to — ADR 0034 dynasty / heir succession
##
## Private helpers (_apply_chain, _faction_director, _resolve_faction_id,
## _tech_tree_director, _dynasty_director) stay internal — they're only
## called from the handlers in this module.
##
## All static. Cross-module: build_place calls EffectCore.spawn (for
## the blueprint spawn) and _apply_chain calls EffectApply.apply
## (for the on_success / on_invalid effect chains).
...
static func _apply_chain(chain, env: Dictionary, ctx: Dictionary) -> void:
	...
	if chain is Dictionary:
		...
		EffectApply.apply(chain, env, ctx)
		return
	if not (chain is Array):
		return
	for sub in chain as Array:
		if sub is Dictionary:
			EffectApply.apply(sub, env, ctx)
...
static func build_place(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
...
static func switch_class(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
...
static func declare_war(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
...
static func sign_treaty(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
	var fd := _faction_director(env)
...
static func propose_alliance(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
	var fd := _faction_director(env)
...
static func swear_loyalty(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
	var fd := _faction_director(env)
...
## tick).
static func try_discover_tech(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
...
static func learn_from_master(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
	var ttd := _tech_tree_director(env)
...
static func pass_to_apprentice(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
	var ttd := _tech_tree_director(env)
	...
	var max_apprentices: int = int(
		EffectResolution.value(e.get("max_apprentices_per_call", 4), ctx, env)
	)
...
static func transfer_inventory(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
...
static func transfer_reputation(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
	var dd := _dynasty_director(env)
...
##    "filter": "core_only"}
static func transfer_techs(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
	var dd := _dynasty_director(env)
...
## impossible.
static func transition_player_to(e: Dictionary, env: Dictionary, ctx: Dictionary) -> Dictionary:
	var dd := _dynasty_director(env)
