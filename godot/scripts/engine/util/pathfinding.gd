...
static func build_mesh_data(walkables: Array, obstacles: Array, floor_y: float) -> NavigationMesh:
	...
	for ix in range(nx):
		for iz in range(nz):
			...
			if not _point_inside_any(center_x, center_z, walkables):
				continue
			if _point_inside_any(center_x, center_z, obstacles):
				continue
...
static func tick_pathfind(
	env: Dictionary, entity, dest_x: float, dest_y: float, dest_z: float, speed: float
) -> void:
	if entity == null:
		return
...
static func _union_xz(rects: Array) -> Dictionary:
	...
	var max_z: float = -INF
	for r in rects:
		if (r as Dictionary)["min_x"] < min_x:
			min_x = (r as Dictionary)["min_x"]
		if (r as Dictionary)["min_z"] < min_z:
			min_z = (r as Dictionary)["min_z"]
		if (r as Dictionary)["max_x"] > max_x:
			max_x = (r as Dictionary)["max_x"]
		if (r as Dictionary)["max_z"] > max_z:
			max_z = (r as Dictionary)["max_z"]
...
static func _ensure_agent(entity, region) -> NavigationAgent3D:
	if entity == null or not (entity is Node):
		return null
