...
static func check(
	...
	match name:
		...
		_:
			push_warning(
				(
					"build_place: unknown predicate '%s' (known: no_overlap, ground_buildable, owner_in_range, boundary_check)"
					% name
				)
			)
...
static func _no_overlap(def: Dictionary, pos: Vector3, yaw: float, env: Dictionary) -> Dictionary:
	...
	for cid in candidate_ids:
		...
		if ent == null or not (ent is Entity):
			continue
		var ent_e: Entity = ent
		if not ent_e.has_tag("blocks_motion"):
			continue
		var other_aabb := _entity_world_aabb(ent_e)
		if other_aabb.is_empty():
			continue
...
static func _ground_buildable(pos: Vector3, env: Dictionary) -> Dictionary:
	...
	for cid in ids:
		...
		var ent_e: Entity = ent
		if not (
			ent_e.has_tag("ground_buildable")
			or ent_e.has_tag("ground")
			or ent_e.has_tag("ground_tile")
		):
			continue
...
static func _owner_in_range(
	owner_binding: String, pos: Vector3, max_range: float, env: Dictionary, ctx: Dictionary
) -> Dictionary:
	if owner_binding == "":
		owner_binding = "self"
	...
	if not entities.has(ent_id) or not (entities[ent_id] is Entity):
		(
			...
			. raise(
				env,
				EngineError.EFFECT_BUILD_PLACE_NO_SOURCE,
				(
					"build_place: owner_in_range predicate found no source entity for binding '%s'"
					% owner_binding
				),
				{
...
static func _boundary_check(pos: Vector3, env: Dictionary) -> Dictionary:
	...
	if min_a.size() >= 2 and max_a.size() >= 2:
		...
		min_z = float(min_a[idx_z])
		max_z = float(max_a[idx_z])
...
static func _build_world_aabb(def: Dictionary, pos: Vector3, yaw: float) -> Dictionary:
	...
	return {
		"minx": center.x - enclosing.x,
		"maxx": center.x + enclosing.x,
		"miny": center.y - enclosing.y,
		"maxy": center.y + enclosing.y,
		"minz": center.z - enclosing.z,
		"maxz": center.z + enclosing.z,
	}
...
static func _aabb_overlap_3d(a: Dictionary, b: Dictionary) -> bool:
	...
	if a.maxy < b.miny or b.maxy < a.miny:
		return false
	if a.maxz < b.minz or b.maxz < a.minz:
		return false
	return true
