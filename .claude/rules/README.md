...
| `validate_ground_shader_params.py` | ground shader uniforms stranded at `ground.mesh` top-level instead of inside `ground.mesh.shader_params`. Parses the referenced `.gdshader` for `uniform` declarations; any that appear at mesh-level but not in shader_params → ERROR (ground_renderer.gd only forwards shader_params; the value is silently ignored and the shader uses its default). Empirical 2026-05-26: `ground.mesh.height_scale=8.0` was ignored, shader ran at its default 2.0; a heightmap re-roll produced zero visible relief change | `.claude/rules/post-mortem.md` (same bug class as state.facing — value written to a key the consumer doesn't read) |
