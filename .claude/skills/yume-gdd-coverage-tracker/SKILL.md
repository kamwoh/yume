...
- **Dynamics → cascades**: every named cascade ("gold spiral",
  ...
  visible in world/rules/*.json feature modules (or legacy single-file
  world/rules.json for smaller games)
...
### Rule subscription
```bash
grep -rB 5 "\"action\": \"<verb>\"" godot/data/demo_<name>/world/rules/ \
  godot/data/demo_<name>/world/rules.json 2>/dev/null
