...
## 2026-05-17 (later session) — Asset-gen pipeline real backends shipped

After the initial framework session above, user picked the two real
backends to wire up:
- **Nanobanana** (a.k.a. Gemini 2.5 Flash Image) for 2D textures +
  concept images
- **Tripo3D** for 3D `.glb` meshes with optional 2D concept-image
  reference

### Shipped (task #118)

- `tools/yume_assetgen/backends/nanobanana.py` — Gemini 2.5 Flash
  Image text-to-image. Multimodal input (text + optional reference
  PNGs). Pure stdlib via urllib. Env: `GEMINI_API_KEY`. Aliased as
  `gemini_image` in REGISTRY.
- `tools/yume_assetgen/backends/tripo3d.py` — Tripo3D async
  text-to-3D + image-to-3D. Submit/poll/download pattern. Image
  upload via STS endpoint with hand-rolled multipart. Env:
  `TRIPO_API_KEY`. Config knobs: poll_interval, timeout, pbr.
- Per-kind backend routing: `asset_gen.json::backend` accepts
  string (legacy, one-for-all) OR dict {texture, mesh, concept}.
  Lazy instantiation so a game that only needs textures doesn't
  require `TRIPO_API_KEY` to be set.
- **Chained mode**: when an entity declares both
  `mesh_reference_prompt` AND `mesh_prompt` under visual:, the
  pipeline runs nanobanana first → concept PNG saved to
  `assets/concepts/<entity>.png` → handed to Tripo3D's
  image-to-3D as the reference. Mesh `.glb` then patched onto
  `visual.mesh`. Concept stays intermediate (not patched).
- `PromptItem` gains `reference_image_path` + `intermediate`
  fields. `scan_prompts` orders concept items BEFORE their consumer
  mesh items so the reference file exists at dispatch time.
- `Backend.generate_mesh` signature extended with optional
  `reference_image: Path | None` (default None).
- MockBackend updated to accept-and-ignore the new kwarg.
- Smoke test expanded: 35 assertions across 8 groups including
  backend routing, real-backend registry presence, chained-mode
  scan, concept PNG verification, idempotent re-run with chain.
- `tools/yume_assetgen/README.md` documents the chain + per-kind
  routing + new starter `asset_gen.json` template.

### Doc + HTML viewer fixes (separate but in same session)

- `.claude/skills/md-to-html/convert.py` — added mermaid client-side
  renderer (UMD build, not ESM, so it works for both file:// AND
  http:// page opens).
- `docs/serve.py` (port 8777 docs server) — same mermaid carve-out
  + UMD script tag, plus: now prefers pre-rendered `.html` (302
  redirect from `.md` URL when a sibling `.html` exists), serves
  static assets (CSS/JS/PNG/etc.), lists all 49 ADRs on the index
  page in a 2-column section.
- ADR 0035 status flipped: proposed → accepted (implementation
  realigned by ADR 0046 — animation primitive).
- ADR 0051 created — Authoring-time Python emitters (codegen +
  assetgen rationale + end-to-end asset-gen flow diagram).
- `docs/33_yume_full_architecture.md` section 13 added covering
  authoring tools; stats refreshed for 51 ADRs / 907 unit tests /
  3 demos / 12 validators.

### Pipeline schema (current shape, possibly to evolve)

Entity-def fields under `visual:` that trigger asset-gen:

| Field | Backend | Produces | Patched onto |
|---|---|---|---|
| `albedo_texture_prompt` | nanobanana | `<game>/assets/textures/<id>.png` | `visual.albedo_texture` |
| `material_overrides.<surface>.albedo_texture_prompt` | nanobanana | (per-surface PNG) | `material_overrides.<surface>.albedo_texture` |
| `mesh_reference_prompt` (intermediate only) | nanobanana | `<game>/assets/concepts/<id>.png` | (none — fed to Tripo3D as ref) |
| `mesh_prompt` | tripo3d | `<game>/assets/meshes/<id>.glb` | `visual.mesh` |

Possible schema evolutions queued for design discussion (NOT
implemented):
- Per-entity `negative_prompt`
- Multi-view references (Tripo3D supports up to 4 views as input)
- Named-variant outputs (e.g. 4 villager skins from one mesh)
- Explicit "regenerate this one only" / `--force-entity <id>` CLI
  flag
- `/yume-design` orchestrator integration — auto-run assetgen
  between content-designer and qa-tester phases

### Where the next session picks up

**State at session close**:
- All code committed; working tree clean.
- Both real backends shipped + smoke-tested with mock (35/35
  PASS). NO REAL API CALL has happened yet — the user wants to
  test live with their keys in the next session.
- Both `GEMINI_API_KEY` + `TRIPO_API_KEY` ARE present in
  `~/.bashrc` (lines 126-127) but blocked from non-interactive
  shells by the `case $- in *i*) ;; *) return;; esac` guard at
  line 8. Two workarounds documented:
    1. Restart Claude Code from a freshly-launched terminal so
       the keys land in the inherited env (cleanest).
    2. Wrap each pipeline call in `bash -ic '...'` to force
       interactive bashrc loading.
- **API keys were printed in plaintext to terminal output during
  diagnosis** — should be rotated before they leak further.
  Google: revoke + reissue at AI Studio; Tripo3D: dashboard.

**First action in the next session** (per user direction at
session close):
1. Pick a small test entity (e.g. Morwen in demo_aldenmere) and
   add 2-3 prompt fields — minimum to validate the live pipeline:
   ```
   visual:
     albedo_texture_prompt: "..."
     mesh_prompt: "..."
   ```
2. Run `--dry-run` to verify assembled prompts before paying.
3. Run for real with `bash -ic 'python3 -m tools.yume_assetgen demo_aldenmere'`.
4. Inspect outputs; verify entity def got patched; relaunch game.
5. If API shape differs from what nanobanana.py / tripo3d.py
   expect, error body is surfaced in the exception — adjust
   field names in the backend file accordingly.

### Tasks shipped this later session

| # | Title | Status |
|---|---|---|
| 118 | Implement real asset-gen backends: nanobanana + tripo3d | ✅ |
| (subtask) | md-to-html + serve.py mermaid rendering (UMD CDN) | ✅ |
| (subtask) | serve.py routing: prefer pre-rendered .html + static asset serving + ADR list on index | ✅ |
| (subtask) | ADR 0051 authoring-time Python emitters | ✅ |
| (subtask) | doc sweep: task_plan, ADR README, CLAUDE.md, framework docs 30/31/33 | ✅ |

Test bench at session close:
- 907/0 Godot unit tests
- 19/19 Aldenmere scenario tests
- 30/30 yume_codegen smoke
- 35/35 yume_assetgen smoke (mock backend only)
